...
export async function getTokenAddressesFromExplorer(
  ...
  try {
    ...
    const tokenTxUrl = `${explorer.baseUrl}?chainid=${explorer.chainId}&action=tokentx&address=${normalizedAddress}&startblock=0&endblock=99999999&sort=asc${explorer.apiKey ? `&apikey=${explorer.apiKey}` : ''}`;
